...
const Home = () => {
    ...
    // Fetch recommended courses based on user preferences, trending courses, or algorithm
    useEffect(() => {
        const fetchRecommendedCourses = async () => {
            try {
                setLoading(true);
                
                // You can implement different recommendation logic here:
                // 1. Based on user's enrolled courses
                // 2. Based on user's search history
                // 3. Trending/popular courses
                // 4. New courses
                // 5. Courses from preferred categories
                
                // Example: Fetch multiple categories and combine them for recommendations
                const categories = [
                    "678efcbd3304458752749d40"," 678efd443304458752749d42","678efd5d3304458752749d44"// Your existing category
                ...
                for (const categoryID of categories) {
                    ...
                    if (result?.selectedCategory?.courses) {
                        allRecommendedCourses = [...allRecommendedCourses, ...result.selectedCategory.courses];
                    }
                    if (result?.mostSellingCourses) {
                        allRecommendedCourses = [...allRecommendedCourses, ...result.mostSellingCourses];
                    }
                }
                
                // Remove duplicates and shuffle for variety
                const uniqueCourses = allRecommendedCourses.filter((course, index, self) => 
                    index === self.findIndex(c => c._id === course._id)
                );
                
                // Shuffle and limit to reasonable number
                const shuffledCourses = uniqueCourses.sort(() => 0.5 - Math.random());
                setRecommendedCourses(shuffledCourses.slice(0, 12)); // Limit to 12 courses
